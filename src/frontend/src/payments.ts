...
export class Payments implements paymentsInterface {
    ...
    async getPackPrices(): Promise<Array<[string, bigint]>> {
        if (this.processError) {
            try {
                const result = await this.actor.getPackPrices();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async setStripeWebhookSecret(arg0: string): Promise<boolean> {
        if (this.processError) {
            try {
...
export function createActor(canisterId: string, _uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, options: CreateActorOptions = {}): Payments {
    ...
    return new Payments(actor, _uploadFile, _downloadFile, options.processError);
}
